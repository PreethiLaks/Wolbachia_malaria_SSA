...
server <- function(input, output, session) {
  ...
  output$map_uc3 <- renderPlot({ res <- tier_weighted(); tr <- if (is.list(res)) res$tier else res; plot_tier(tr, "") })
