...
# Pre-computed default tier rasters — load instantly
DEFAULT_TIERS <- lapply(setNames(sp_names, sp_names), function(sp) {
  rast(file.path(data_dir, paste0("tier_uc1a_", sp, ".tif")))
})

# Species aligned rasters — loaded on demand when thresholds change
sp_path <- list(
...
ui <- fluidPage(
  ...
  titlePanel(
    div(
      h3("Demonstration trial 1a — Entomological feasibility"),
      p(style = "color:#666; font-size:0.9em; margin-top:-8px;",
        "Spatial eligibility for Wolbachia entomological demonstration | ",
  ...
  sidebarLayout(
    ...
    mainPanel(width = 9,
              tabsetPanel(
                tabPanel("Map",
                         plotOutput("map_uc1a", height = 650)
                ),
...
server <- function(input, output, session) {
  ...
  output$sens_plot <- plotly::renderPlotly({
    ...
    # Compute % change from baseline for each species
    df_plot <- do.call(rbind, lapply(unique(df$species), function(sp) {
      d        <- df[df$species == sp, ]
      baseline <- d$area_km2[which.min(abs(d$threshold - baseline_th))]
      d$pct_change <- round(100 * (d$area_km2 - baseline) / baseline, 1)
      d$y_log      <- d$pct_change + 100  # shift so 0% = 100 on log scale
      d
    }))
    ...
    for (sp in unique(df_plot$species)) {
      ...
      fig <- plotly::add_trace(fig,
                               ...
                               hovertemplate = paste0(
                                 "<b>", sp, "</b><br>",
                                 x_label, ": %{x}%<br>",
                                 "Change from baseline: %{text}<extra></extra>"
                               )
      )
    }
